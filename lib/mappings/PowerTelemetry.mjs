...
export class PowerTelemetry {
  ...
  static async add({ device, parent, registerEndpoint, makeCapabilityInstance, subscriptions }) {
    ...
    const power = capabilities.measure_power && !subscriptions?.measure_power;

    if (imported || exported || power) {
      const servers = [PowerTopologyServer.with('TreeTopology')];
      const properties = { id: 'energy' };
      ...
      if (power) {
        ...
        properties.electricalPowerMeasurement = {
          ...
          accuracy: [PowerTelemetry.accuracy(MeasurementType.ActivePower)],
          activePower: null,
        };
      }
      ...
      if (power) {
        makeCapabilityInstance('measure_power', async (value) => {
          const activePower = typeof value === 'number' && Number.isFinite(value)
            ? Math.round(value * 1000)
            : null;
          await endpoint.set({ electricalPowerMeasurement: { activePower } });
        });
      }
