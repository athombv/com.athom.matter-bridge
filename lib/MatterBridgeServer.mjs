import { CapabilityGroups } from './mappings/CapabilityGroups.mjs';
import { FanMapping } from './mappings/FanMapping.mjs';
import { PowerTelemetry } from './mappings/PowerTelemetry.mjs';
import { HomeyTransitions } from './mappings/HomeyTransitions.mjs';
...
import { MeasurementType, Status, StatusResponseError } from '@matter/main/types';

export class MatterBridgeServer {
  // Homey exposes normalized color temperature without physical limits. Use Homey OS's fallback.
  static #minColorTemperatureMireds = 153;
  static #maxColorTemperatureMireds = 400;
  ...
  static getDeviceSupport(device) {
    ...
    for (const group of CapabilityGroups.forDevice(device)) {
      const support = MatterBridgeServer.#getGroupSupport(group.device);

      for (const id of support.supportedCapabilities) {
        supported.add(group.ids[id]);
      }
    }
    ...
    for (const id of Object.keys(device.capabilitiesObj ?? {})) {
      if (supported.has(id)) {
    ...
    return { canShare: supported.size > 0, supportedCapabilities, unsupportedCapabilities };
  }

  static #getGroupSupport(device) {
    const capabilities = device.capabilitiesObj ?? {};
    const deviceClass = device.virtualClass || device.class;
    const candidates = new Set([
      'measure_temperature', 'measure_humidity', 'measure_co', 'measure_co2', 'measure_pm1', 'measure_pm10',
      'measure_pm25', 'measure_luminance', 'alarm_motion', 'alarm_occupancy', 'alarm_contact',
      'alarm_smoke', 'alarm_water', 'measure_power', 'measure_voltage', 'measure_current',
    ...
    switch (deviceClass) {
      ...
      case 'curtain': {
        ...
        if (capabilities.windowcoverings_set && capabilities.windowcoverings_state) {
          candidates.add('windowcoverings_state');
        }
        break;
      }
      default: {
    ...
    return await this.__queue.add(async () => {
      ...
      const makeCapabilityInstance = (capabilityId, callback) => {
        ...
        const update = async (value) => {
          ...
          const requiresBoolean = MatterBridgeServer.#capabilitiesRequiringKnownBoolean.includes(capabilityId.split('.')[0]);
      ...
      for (const group of CapabilityGroups.forDevice(device)) {
        ...
        const subscribe = (id, callback) => {
          const sourceId = group.ids[id];

          if (!sourceId) {
            return;
          }
          subscriptions[id] = true;
          makeCapabilityInstance(sourceId, callback);
        };
        const endpointId = (id) => {
          return group.suffix ? `channel:${encodeURIComponent(group.suffix)}:${id}` : id;
        };

        await this.#addCapabilityGroup({
          device: group.device,
          parent: deviceEndpoint,
          registerEndpoint,
          makeCapabilityInstance: subscribe,
          subscriptions,
          endpointId,
        });
      }
  ...
  async #addCapabilityGroup({ device, parent, registerEndpoint, makeCapabilityInstance, subscriptions, endpointId }) {
    class HomeyScenesManagementServer extends ScenesManagementServer {
      async recallScene(request) {
        this.internal.sourceError = undefined;
        await super.recallScene(request);

        // Matter.js logs and swallows apply-callback failures. Preserve the source error so the
        // controller cannot report a successful recall when the physical device rejected it.
        const error = this.internal.sourceError;
        this.internal.sourceError = undefined;
      ...
      implementScenes(behavior, fallback) {
        const cluster = behavior.type.id;

        super.implementScenes(behavior, async function applySourceScene(values, transitionTime) {
          const writes = {};

          if (cluster === 'onOff' && values.onOff !== undefined) {
            writes.onoff = values.onOff;
          } else if (cluster === 'levelControl' && values.currentLevel !== undefined) {
            writes.dim = MatterBridgeServer.#scaleNumber(values.currentLevel, 1, 254, 0, 1);
          } else if (cluster === 'colorControl') {
            const mode = values.enhancedColorMode ?? this.state.enhancedColorMode;
            const temperatureMode = mode === ColorControl.EnhancedColorMode.ColorTemperatureMireds;
            writes.light_mode = temperatureMode ? 'temperature' : 'color';

            if (temperatureMode && values.colorTemperatureMireds !== undefined) {
              writes.light_temperature = MatterBridgeServer.#scaleNumber(
                values.colorTemperatureMireds, MatterBridgeServer.#minColorTemperatureMireds,
                MatterBridgeServer.#maxColorTemperatureMireds, 0, 1,
              );
            } else if (values.currentSaturation !== undefined) {
              writes.light_saturation = values.currentSaturation / 254;
            }
          } else if (cluster !== 'onOff' && cluster !== 'levelControl') {
            return await fallback.call(this, values, transitionTime);
          }

          this.internal.transitions?.stop();
          try {
            await MatterBridgeServer.#setSupportedCapabilities(device, writes, transitionTime / 100);
          } catch (error) {
            this.agent.get(ScenesManagementServer).internal.sourceError ??= error;
            throw error;
          }
        });
      }
    ...
    switch (deviceClass) {
      case 'socket': {
        class HomeyOnOffServer extends OnOffServer {
          initialize() {
            super.initialize();
            this.agent.maybeGet(ScenesManagementServer)?.implementScenes(this, () => {});
          }
        ...
        const endpoint = new Endpoint(
          OnOffPlugInUnitDevice.with(...endpointServers, HomeyScenesManagementServer),
          endpointProperties,
        );
      ...
      case 'light': {
        ...
        class HomeyLevelControlServer extends LevelControlServer {
          async transition(targetLevel, changePerS, withOnOff = false, options = {}) {
            targetLevel = Math.max(this.minLevel, Math.min(this.maxLevel, targetLevel));
            this.internal.sourceWithOnOff = withOnOff;
            await this.couple(false, options, targetLevel);

            if (typeof changePerS === 'function') {
              changePerS = changePerS(this.currentLevel);
            }

            await this.internal.transitions.start({
              name: 'currentLevel', owner: this, changePerS, targetValue: targetLevel,
            });
          }

          createTransitions(config) {
            return new HomeyTransitions(this.endpoint, config, async (behavior, changes) => {
              const values = {};

              if (changes.currentLevel !== undefined) {
                values.dim = MatterBridgeServer.#scaleNumber(changes.currentLevel, 1, 254, 0, 1);
              }
              if (changes.currentLevel !== undefined && device.capabilitiesObj.onoff && behavior.internal.sourceWithOnOff) {
                const onoff = changes.currentLevel > behavior.minLevel;

                if (device.capabilitiesObj.onoff.value !== onoff) {
                  values.onoff = onoff;
                }
              }

              await MatterBridgeServer.#setSupportedCapabilities(device, values, 0);

              if (values.onoff !== undefined) {
                behavior.agent.get(OnOffServer).state.onOff = values.onoff;
              }
            });
          }
          ...
          async stopLogic() {
            this.internal.transitions.stop();
            await device.setCapabilityValue({
              capabilityId: 'dim', value: device.capabilitiesObj.dim.value, opts: { duration: 0 },
            });
          }

          async moveToLevelWithOnOff({ level, transitionTime }) {
            this.internal.transitions.stop();
            await MatterBridgeServer.#setSupportedCapabilities(device, {
              onoff: level > 0,
              dim: MatterBridgeServer.#scaleNumber(level, 1, 254, 0, 1),
            }, transitionTime ?? this.state.onOffTransitionTime ?? 0);
          }

          async moveToLevel({ level, transitionTime }) {
            this.internal.transitions.stop();
            await MatterBridgeServer.#setSupportedCapabilities(device, {
              dim: MatterBridgeServer.#scaleNumber(level, 1, 254, 0, 1),
            }, transitionTime ?? this.state.onOffTransitionTime ?? 0);
        ...
        class HomeyColorControlServer extends ColorControlServer {
          createTransitions(config) {
            return new HomeyTransitions(this.endpoint, config, async (behavior, changes) => {
              ...
              if (changes.currentHue !== undefined) {
                values.light_hue = changes.currentHue / 254;
              }
              if (changes.currentSaturation !== undefined) {
                values.light_saturation = changes.currentSaturation / 254;
              }
              if (changes.colorTemperatureMireds !== undefined) {
                values.light_temperature = MatterBridgeServer.#scaleNumber(
                  changes.colorTemperatureMireds,
                  MatterBridgeServer.#minColorTemperatureMireds,
                  MatterBridgeServer.#maxColorTemperatureMireds, 0, 1,
                );
              }
              const mode = behavior.state.colorMode === ColorControl.ColorMode.ColorTemperatureMireds
                ? 'temperature' : 'color';

              if (device.capabilitiesObj.light_mode?.value !== mode) {
                values.light_mode = mode;
              }
          ...
          setColorMode(mode) {
            // The source owns inactive color values; switching modes must not synthesize new ones.
          ...
          async moveHue(request) {
            if (request.moveMode !== ColorControl.MoveMode.Stop) {
              return await super.moveHue(request);
            }
            if (this.state.colorMode === ColorControl.ColorMode.CurrentHueAndCurrentSaturation) {
              await this.stopMoveStep(request);
            }
          }

          async moveSaturation(request) {
            if (request.moveMode !== ColorControl.MoveMode.Stop) {
              return await super.moveSaturation(request);
            }
            if (this.state.colorMode === ColorControl.ColorMode.CurrentHueAndCurrentSaturation) {
              await this.stopMoveStep(request);
            }
          ...
          async moveColorTemperature(request) {
            if (request.moveMode !== ColorControl.MoveMode.Stop) {
              return await super.moveColorTemperature(request);
            }
            if (this.state.colorMode === ColorControl.ColorMode.ColorTemperatureMireds) {
              await this.stopMoveStep(request);
            }
          }

          async stopHueAndSaturationMovement() {
            super.stopHueAndSaturationMovement();
            await MatterBridgeServer.#setSupportedCapabilities(device, {
          ...
          async stopAllColorMovement() {
            this.internal.transitions.stop();
            const values = this.state.colorMode === ColorControl.ColorMode.ColorTemperatureMireds
          ...
          async stopMoveStepLogic() {
            await this.stopAllColorMovement();
          }

          initialize() {
            // Older mappings could persist values outside today's physical limits. Refresh source
            // readings before Matter validates the restored cluster or the whole endpoint is lost.
            for (const name of ['currentHue', 'currentSaturation', 'colorTemperatureMireds']) {
              const value = endpointProperties.colorControl[name];
          ...
          async moveToHueAndSaturation({ hue, saturation, transitionTime }) {
            this.internal.transitions.stop();
            await MatterBridgeServer.#setSupportedCapabilities(device, {
              onoff: true,
          ...
          async moveToColorTemperature({ colorTemperatureMireds, transitionTime }) {
            this.internal.transitions.stop();
            await MatterBridgeServer.#setSupportedCapabilities(device, {
              onoff: true,
              light_temperature: MatterBridgeServer.#scaleNumber(
                ...
                MatterBridgeServer.#minColorTemperatureMireds,
                MatterBridgeServer.#maxColorTemperatureMireds,
                0,
                1,
              ),
        ...
        if (supportsColorTemperature) {
          endpointProperties.colorControl = {
            ...
            colorTempPhysicalMinMireds: MatterBridgeServer.#minColorTemperatureMireds,
            colorTempPhysicalMaxMireds: MatterBridgeServer.#maxColorTemperatureMireds,
            coupleColorTempToLevelMinMireds: MatterBridgeServer.#minColorTemperatureMireds,
          };
          ...
          makeCapabilityInstance('light_temperature', async (value) => {
            await endpoint.set({
              colorControl: {
                colorTemperatureMireds:
                  MatterBridgeServer.#scaleAndRoundNumber(value, 0, 1,
                    MatterBridgeServer.#minColorTemperatureMireds,
                    MatterBridgeServer.#maxColorTemperatureMireds) ?? 277,
              },
            });
          });
        }
        ...
        const endpoint = new Endpoint(endpointClass.with(...endpointServers, HomeyScenesManagementServer), endpointProperties);
        await registerEndpoint(endpoint);
      ...
      case 'airconditioning': {
        ...
        if (device.capabilitiesObj?.measure_humidity) {
          const endpoint = new Endpoint(
            ...
            {
              id: endpointId('measure_humidity'),
      ...
      case 'lock': {
        if (device.capabilitiesObj?.locked) {
          ...
          class HomeyDoorLockServer extends DoorLockServer.with().enable({
            commands: { unlockWithTimeout: false },
          }) {
            unlockWithTimeout = undefined;
            async lockDoor() {
              await device.setCapabilityValue({
                capabilityId: 'locked',
                value: true,
      ...
      case 'curtain': {
        if (device.capabilitiesObj?.windowcoverings_set) {
          const HomeyWindowConveringServer = class extends WindowCoveringServer.with(
            ...
            async stopMotion() {
              if (!device.capabilitiesObj.windowcoverings_state) {
                throw new StatusResponseError('Source cover has no stop capability', Status.InvalidInState);
              }

              await device.setCapabilityValue({ capabilityId: 'windowcoverings_state', value: 'idle' });
            }
          };
          ...
          if (device.capabilitiesObj.windowcoverings_state) {
            makeCapabilityInstance('windowcoverings_state', async (value) => {
              ...
              if (movement !== undefined) {
                await endpoint.set({ windowCovering: { operationalStatus: { global: movement, lift: movement } } });
              }
            });
          }
          ...
          const HomeyWindowConveringServer = class extends WindowCoveringServer.with(
            ...
            async goToLiftPercentage() {
              throw new StatusResponseError('Source cover has no position capability', Status.InvalidInState);
            }
          };
      ...
      case 'fan': {
        const added = await FanMapping.add({ device, registerEndpoint, makeCapabilityInstance, endpointId });
      ...
      default: {
        ...
        if (device.capabilitiesObj?.onoff) {
          ...
          const endpoint = new Endpoint(OnOffPlugInUnitDevice.with(HomeyOnOffServer, HomeyScenesManagementServer), {
            id: endpointId('main'),
            onOff: {
              onOff: device.capabilitiesObj?.onoff?.value ?? false,
            },
    ...
    if (
      ...
      !subscriptions?.measure_humidity
    ) {
      const endpoint = new Endpoint(
        HumiditySensorDevice.with(RelativeHumidityMeasurementServer),
        {
    ...
    if (
      device.capabilitiesObj?.measure_pm1 &&
      !subscriptions?.measure_pm1
    ) {
      const endpoint = new Endpoint(
        AirQualitySensorDevice.with(
          Pm1ConcentrationMeasurementServer.with('NumericMeasurement'),
        ),
        {
          id: endpointId('measure_pm1'),
          airQuality: { airQuality: AirQuality.AirQualityEnum.Unknown },
          pm1ConcentrationMeasurement: {
            measurementUnit: ConcentrationMeasurement.MeasurementUnit.Ugm3,
            measurementMedium: ConcentrationMeasurement.MeasurementMedium.Air,
            measuredValue:
              typeof device.capabilitiesObj?.measure_pm1?.value === 'number'
                ? device.capabilitiesObj.measure_pm1.value
                : null,
          },
        },
      );
      ...
      makeCapabilityInstance('measure_pm1', async (value) => {
        await endpoint.set({
          pm1ConcentrationMeasurement: {
            measuredValue: typeof value === 'number' ? value : null,
          },
    ...
    if (
      ...
      !subscriptions?.measure_luminance
    ) {
      const endpoint = new Endpoint(LightSensorDevice, {
        id: endpointId('measure_luminance'),
  ...
  async #updateDeviceReachability(device) {
    ...
    const unknownReading = Object.keys(subscriptions).some((id) => {
      return MatterBridgeServer.#capabilitiesRequiringKnownBoolean.includes(id.split('.')[0]) &&
        typeof device.capabilitiesObj[id]?.value !== 'boolean';
    });
    const deviceClass = device.virtualClass || device.class;
    const unknownSpeed = deviceClass === 'fan' && Object.keys(subscriptions).some((id) => {
      return id.split('.')[0] === 'fan_speed' && !Number.isFinite(device.capabilitiesObj[id].value);
    });
  ...
  static async #setSupportedCapabilities(device, values, transitionTime) {
    const writePromises = [];
    ...
    for (const [capabilityId, value] of Object.entries(values)) {
      ...
      const write = { capabilityId, value };
      const supportsTransition = [
        'onoff', 'dim', 'light_hue', 'light_saturation', 'light_temperature',
      ].includes(capabilityId.split('.')[0]);

      if (supportsTransition && transitionTime != null) {
        // Matter uses deciseconds; Homey capability listeners receive milliseconds.
        write.opts = { duration: transitionTime * 100 };
      }

      writePromises.push(device.setCapabilityValue(write));
    }
