...
export default class MatterBridgeApp extends Homey.App {
  ...
  async onAPIGetDevices() {
    ...
    for (const device of Object.values(devices)) {
      ...
      const describeCapability = (id) => {
        return { id, title: device.capabilitiesObj[id].title ?? id.replaceAll('_', ' ') };
      };
      deviceObj.support = {
        canShare: support.canShare,
        supportedCapabilities: support.supportedCapabilities.map(describeCapability),
        unsupportedCapabilities: support.unsupportedCapabilities.map(describeCapability),
      };
  ...
  async onAPIStartPairing() {
    if (!this.server) throw new Error('Server Not Ready');
    return await this.server.startPairing();
  }

  async onAPIStopPairing() {
    if (!this.server) throw new Error('Server Not Ready');
    return await this.server.stopPairing();
  }

  async onAPIEnableDevice({ deviceId }) {
    await this.#selectionQueue.add(async () => {
      const enabledDeviceIds = await this.getEnabledDeviceIds();
